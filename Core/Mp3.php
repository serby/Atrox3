...
/**
 * The path to the mpgtx application used for splicing and modifing mpgs
 */
define("MP3_MPGTXPATH", "/usr/bin/mpgtx");

/**
 * ID3V2 used for editing ID3 information
 */
define("MP3_ID3V2PATH", "/usr/bin/id3v2");

/**
 * ID3 Tags Fields
 */
define("MP3_ID3_ARTIST", 1);
define("MP3_ID3_ALBUM", 2);
define("MP3_ID3_SONG", 3);
define("MP3_ID3_COMMENT", 4);
define("MP3_ID3_GENRE", 5);
define("MP3_ID3_YEAR", 6);
define("MP3_ID3_TRACK", 7);
...
class Mp3Control {
	...
	function Mp3Control() {
		if (!file_exists(MP3_MPGTXPATH)) {
			trigger_error(MP3_MPGTXPATH . " not found. MP3Control's functionality " .
				"will be restricted with it.");
		}
		if (!file_exists(MP3_ID3V2PATH)) {
			trigger_error(MP3_ID3V2PATH . " not found. MP3Control's functionality " .
				"will be restricted with it.");
		}			
	}

	function mb_split($infile, $outfile, $random = false, $in = 1, $out = 30) {
		$id3 = $this->getId3($infile);

		$out = min($id3->lengths, $out);
		...
		if ($random) {
			$randomWindow = $id3->lengths - ($out - $in);
			$randomShift = rand(1, $randomWindow);
			$in += $randomShift;
			$out += $randomShift;
		}

		$inSeconds = $in % 60;
		$inMinutes = floor(($in - $inSeconds) / 60) % 60;
		$inHours = ($in - $inSeconds - $inMinutes * 60) / 3600;

		$outSeconds = $out % 60;
		$outMinutes = floor(($out - $outSeconds) / 60) % 60;
		$outHours = ($out - $outSeconds - $outMinutes * 60) / 3600;

		$inTime = $inHours . ":" . str_pad($inMinutes, 2, "0", STR_PAD_LEFT) . ":" . str_pad($inSeconds, 2, "0", STR_PAD_LEFT);
		$outTime = $outHours . ":" . str_pad($outMinutes, 2, "0", STR_PAD_LEFT) . ":" . str_pad($outSeconds, 2, "0", STR_PAD_LEFT);

		exec(MP3_MPGTXPATH . " -s -fN \"$infile\" [$inTime-$outTime] -o \"$outfile\"");
	}

	function getId3($filename, $study = true) {
		return new Id3Control($filename, $study);
	}

	function join($file1, $file2, $outfile) {
		exec(MP3_MPGTXPATH . " --force -j -fN \"$file1\" \"$file2\" -o \"$outfile\"");
	}
	
	function setId3($filename, $type, $value) {
		...
		exec(MP3_ID3V2PATH . " $typeArgument \"$value\" \"$filename\"");
	}		
}
...
class Id3Control {
		...
		*/
		function Id3Control($file, $study = false) {
		...
		*/
		...
		if ($genre) {
				...
				if (mb_strtolower($genre) == mb_strtolower($name)) {
